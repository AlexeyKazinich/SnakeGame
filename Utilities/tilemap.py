...
class Tile:
    ...
    def draw(self, offset: tuple = (0, 0), color: pygame.Color = pygame.Color(0, 0, 0, 255)) -> None:
        ...
        if self.fill_type == "fill":
            self.rect = Rectangle((int(self.pos[0]) + int(offset[0])) * self.tile_size,
                                  (int(self.pos[1]) + int(offset[1])) * self.tile_size, self.tile_size, self.tile_size,
                                  self.screen, self.color)
            self.rect.draw()
        elif self.fill_type == "outline":
            self.rect = Rectangle((self.pos[0] + int(offset[0])) * self.tile_size,
                                  (self.pos[1] + offset[1]) * self.tile_size, self.tile_size, self.tile_size,
                                  self.screen, color)
            self.rect.draw_box()
...
class Tilemap:
    ...
    def extend_snake(self) -> None:
        ...
        if self.snake_length == 1:
            self.snake[f"body{self.snake_length}"] = Tile(self.screen, self.get_loc_part("head"), fill_type="fill",
                                                          color=self.body_color, tile_size=self.tile_size)
            self.snake_length += 1
        else:
            self.snake[f"body{self.snake_length}"] = Tile(self.screen,
                                                          self.get_loc_part(f"body{self.snake_length - 1}"),
                                                          fill_type="fill", color=self.body_color,
    ...
    def update_apple(self) -> None:
        ...
        self.apple = Tile(self.screen, (randint(0, self.grid_size[0] - 1), randint(0, self.grid_size[1] - 1)),
                          self.tile_size, "fill", self.apple_color)
